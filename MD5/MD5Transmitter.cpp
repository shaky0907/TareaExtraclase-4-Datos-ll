...
// Default Constructor, just initialize
MD5Transmitter::MD5Transmitter() {
    init();
}

// Constructor with string parameter and transform immediately
MD5Transmitter::MD5Transmitter(const std::string& str) {
    init();
    update(str.c_str(), str.length());
    finalize();
}

// MD5Transmitter block update operation. Continues an MD5Transmitter message-digest
// operation, processing another message block
void MD5Transmitter::update(const uChar input[], uInt length) {
...
// MD5Transmitter block update operation with unsigned char
void MD5Transmitter::update(const char input[], uInt length) {
    update((const uChar*)input, length);
}

// MD5Transmitter finalization. Ends an MD5Transmitter message-digest operation,
// writing the message digest and zeroizing the context
MD5Transmitter& MD5Transmitter::finalize() {
...
std::string MD5Transmitter::hexdigest() {
    if (!finish) {
        return "";
    }
    char temp[33] = {0};
...
// Ouput stream 
std::ostream& operator << (std::ostream& out, MD5Transmitter md5) {
    return out << md5.hexdigest();
}

// Initialization
void MD5Transmitter::init() {
    finish = false;
    count[0] = count[1] = 0;
    // Magic number
    state[0] = 0x67452301;
...
// MD5Transmitter transform operation.
void MD5Transmitter::transform(const uChar block[blockSize]) {
...
void MD5Transmitter::decode(uInt output[], const uChar input[], uInt length) {
...
void MD5Transmitter::encode(uChar output[], const uInt input[], uInt length) {
    for (uInt i = 0, j = 0; j < length; i++, j += 4) {
...
// F, G， H，I are the basic MD5Transmitter functions
...
// md5 function use the parameter to construct a MD5Transmitter object
// and return the MD5Transmitter value
std::string MD5Transmitter::md5(const std::string str) {
    return MD5Transmitter(str).hexdigest();
}